...
function identityFor(source){const [w,h]=source.native_size,s=info.strip,scale=Math.min(s.width/w,s.height/h)*.45;return [scale,0,(s.width-w*scale)/2,0,scale,(s.height-h*scale)/2,0,0,1]}
...
function renderLayers(){const list=$('layers');list.innerHTML=layers.length?layers.map(l=>`<button class="layer ${l.id===selectedId?'active':''}" data-layer="${l.id}">${l.name} ${l.rectified?'':'(raw)'}</button>`).join(''):'<span class="status">No deployed pair sources are available in bench mode.</span>';list.querySelectorAll('[data-layer]').forEach(b=>b.onclick=()=>selectLayer(b.dataset.layer))}
function configureCanvas(){const c=$('mosaicCanvas'),maxW=1200,maxH=820,scale=Math.min(maxW/info.strip.width,maxH/info.strip.height);canvasScale=scale;c.width=Math.max(1,Math.round(info.strip.width*scale));c.height=Math.max(1,Math.round(info.strip.height*scale));c.style.width=c.width+'px';c.style.height=c.height+'px'}
...
function sameMatrix(left,right){return left.length===right.length&&left.every((value,index)=>Math.abs(value-right[index])<0.00001)}async function saveStripLayout(){const status=$('calibrationStatus'),button=$('saveAll');if(!layers.length){status.textContent='No available source layers to save.';return}button.disabled=true;status.textContent=`Save clicked. Writing ${layers.length} strip transforms...`;try{const submitted=layers.map(l=>({pair:l.pair,source:l.source,matrix:l.matrix}));const r=await api('/api/calibration/transforms',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({revision,transforms:submitted})});revision=r.revision;const verified=submitted.every(t=>sameMatrix(t.matrix,r.saved[`${t.pair}:${t.source}`]||[]));status.textContent=verified?`Persisted ${r.count} strip transforms to ${r.layout_path}. Read-back verified.`:'Save response did not match submitted strip transforms; do not enable deployment.'}catch(e){status.textContent=`Save failed: ${e.message}`}finally{button.disabled=false}}$('saveAll').addEventListener('click',saveStripLayout);
...
def create_app(mode):
    ...
    def save_transforms():
        ...
        with layout_lock:
            ...
            candidate["calibrated"] = False
            save_layout(candidate)
            persisted = load_json(LAYOUT_PATH)
            saved = {}
            for record in records:
